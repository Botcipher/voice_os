...
// Set in Retell → Webhook Settings → Agent Level Webhook URL:
...
router.post('/webhooks/retell', async (req, res) => {
  ...
  if (event === 'call_started') {
    ...
    if (!toNumber) return res.status(204).send();

    const result = await getTenantByPhone(toNumber);
    if (!result) {
      console.warn('[call_started] No tenant for:', toNumber);
    ...
    const dynamicVariables = {
      ...
      emergency_callback_minutes: String(s.emergency_callback_minutes || 30),
    };
    ...
    try {
      ...
      else console.log('[call_started] Variables injected for tenant:', tenant.id);
    } catch (err) {
      console.error('[call_started] Error:', err.message);
    }
    return res.status(204).send();
  }
  ...
  if (event === 'call_ended' || event === 'call_analyzed') {
    ...
    if (retellCallId) {
      ...
      if (existing) { console.log(`[${event}] Duplicate — skipping`); return res.status(204).send(); }
    }

    const name        = call.retell_llm_dynamic_variables?.caller_name || null;
    const jobType     = call.call_analysis?.custom_analysis_data?.job_type || null;
    const urgency     = call.call_analysis?.custom_analysis_data?.urgency || 'normal';
    const address     = call.call_analysis?.custom_analysis_data?.address || null;
    const notes       = call.call_analysis?.custom_analysis_data?.notes || null;
    ...
    const { lead } = await upsertLead(tenantId, { phone, name, jobType, urgency, address, notes, source: 'call' });

    await saveCall(tenantId, lead.id, {
      retell_call_id: retellCallId, call_status: callStatus,
...
router.get('/settings/:tenant_id', async (req, res) => {
  const settings = await getSettings(req.params.tenant_id);
  return res.json(settings);
});

router.put('/settings/:tenant_id', async (req, res) => {
  const updated = await updateSettings(req.params.tenant_id, req.body);
...
router.patch('/dashboard/leads/:id/assign-human', async (req, res) => {
  const updated = await assignToHuman(req.params.id);
